...
async def init_project(project_id):
    ...
    client.start(thread=False)  # Use non-threaded mode with asyncio
    projects[project_id] = client

# Add projects
async def add_projects(project_ids):
    tasks = [init_project(project_id) for project_id in project_ids]
...
asyncio.run(add_projects(PROJECT_IDS))
